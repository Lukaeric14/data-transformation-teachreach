...
class OpenAIClient:
    ...
    def _create_multi_field_prompt(self, field_names: List[str], teacher_data: Dict[str, Any]) -> str:
        ...
        field_descriptions = {
            "Years of experience (P)": "the years of teaching experience as a number",
            "Subject (Array) (c)": "the subjects they teach (up to 3, comma-separated)",
            "Preferred curriculumn (O)": "their preferred curriculum (IB, British, American, SABIS, or Other)",
            "Nationality (Z)": "their nationality (e.g., 'British', 'American')"
        }
        
        fields_text = ", ".join([f"'{field}'" for field in field_names])
        field_instructions = "\n".join([
            f"- {field}: {field_descriptions.get(field, 'infer this value')}"
            for field in field_names
        ])
        
        prompt = f"""Based on the following teacher profile, infer these fields: {fields_text}.
